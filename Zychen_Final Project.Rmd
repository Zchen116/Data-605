...
```
#So, P(X > x and Y > y) = 0.3723 and P(X>x)P(Y>y) = 0.375. They are almost equal. 
...
#After eliminating th incomplte data we have the following columns that werer elevant for our analysis.  
```{r}
colnames(md)
```
...
#we can see categorical variables were converted to numerical values and NAs were replaced with zeros for convenience .
```{r}
...
#Here I have Log-transform sales price in the training set for the purpose of training and testing sets .
```{r}
...
#To have better understanding, I have decided using `stepAIC` method.
```{r}
...
#from the graph, we can see obsreved residuals are  normally distributed. 
```{r}
...
#My Kaggle username is **zchen116**. My Score is **0.13513**
